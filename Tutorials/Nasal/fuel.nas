...
var fuel_pressure = func(tank, engine){
  ...
  var fuel_warning_flag = "sim/messages/flags/engine["~engine~"]/fuel-warning";
  var eng_state = getprop("engines/engine["~to_engine~"]/running");

  if(boost_pump > 8){
    if(fuel_qty >= 10) interpolate(fuel_psi, 16, 3);
    elsif(fuel_qty < 10) interpolate(fuel_psi, fuel_qty*1.6, 3);
  }else{
    if(eng_state){
      if(fuel_qty >= 10) interpolate(fuel_psi, 14.5, 3);
      elsif(fuel_qty < 10) interpolate(fuel_psi, fuel_qty*1.45, 3);
    }else{
      interpolate(fuel_psi, 0, 3);
    }
  }

  var fuel_warning = getprop(fuel_warning_flag) or 0;
  if (!fuel_warning and eng_state and getprop(fuel_psi) < 13.0) {
    setprop("sim/messages/copilot", "Fuel pressure is low on engine "~engine);
    setprop(fuel_warning_flag, 1);
  }
  if (fuel_warning and getprop(fuel_psi) > 14.0) {
    setprop("sim/messages/copilot", "Fuel pressure restored on engine "~engine);
    setprop(fuel_warning_flag, 0);
  }
